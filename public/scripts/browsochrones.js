(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.Browsochrones = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
(function (global){
/*!
...
exports.default = accessibilityForGrid;
/**
...
function accessibilityForGrid(_ref) {
  var _ref$cutoff = _ref.cutoff;
  var cutoff = _ref$cutoff === undefined ? 60 : _ref$cutoff;
...
var _lodash = require('lodash.fill');
...
function getSurface(_ref) {
  ...
  var nMinutes = origin.data[transitOffset + 1];
  ...
  for (var y = 0, pixelIdx = 0, stcOffset = 0; y < query.height; y++) {
    for (var x = 0; x < query.width; x++, pixelIdx++) {
      var nStops = stopTreeCache.data[stcOffset++];

      // can we reach this pixel without riding transit?
      var nonTransitTime = (0, _origin.getNonTransitTime)(origin, { x: x, y: y });
      ...
      for (var stopIdx = 0; stopIdx < nStops; stopIdx++) {
        // read the stop ID
        var stopId = stopTreeCache.data[stcOffset++];

        // read the time (minutes)
        var time = stopTreeCache.data[stcOffset++];

        for (var minute = 0; minute < nMinutes; minute++) {
          var offset = origin.index[stopId] + minute * _origin.ITERATION_WIDTH;
          var travelTimeToStop = origin.data[offset];

          if (travelTimeToStop !== -1) {
            var travelTimeToPixel = travelTimeToStop + time;
...
function getTransitOffset(radius) {
...
function computePixelValue(which, travelTimes) {
  switch (which) {
    case 'BEST_CASE':
...
function computeBestPixelValue(travelTimes) {
  var pixel = 255;
...
function computeWorstPixelValue(travelTimes) {
  var pixel = 0;
  for (var i = 0; i < travelTimes.length; i++) {
...
exports.default = getTransitiveData;
...
var _lodash3 = require('lodash.slice');
...
var debug = (0, _debug2.default)('browsochrones:get-transitive-data');
...
var MAXIMUM_DISSIMILARITY = 5e-6;

function getTransitiveData(_ref) {
  var origin = _ref.origin;
  var query = _ref.query;
  var stopTreeCache = _ref.stopTreeCache;
  ...
  output.places.push({
    ...
    place_lat: (0, _mercator.pixelToLat)(query.north + origin.y, query.zoom),
    place_lon: (0, _mercator.pixelToLon)(query.west + origin.x, query.zoom)
  });

  output.places.push({
    place_id: 'to',
    place_name: to.name || 'Destination',
    place_lat: (0, _mercator.pixelToLat)(query.north + to.y, query.zoom),
    place_lon: (0, _mercator.pixelToLon)(query.west + to.x, query.zoom)
  });

  // find relevant paths at each minute
  // this array stores the destination stop and the path index in that stop

  var _getPaths = getPaths({ origin: origin, query: query, stopTreeCache: stopTreeCache, to: to });

  var paths = _getPaths.paths;
  ...
  paths.sort(function (p1, p2) {
    var v0 = p1[0] - p2[0]; // sort first by stop ID
    if (v0 !== 0) return v0;else return p1[1] - p2[1]; // then by path index
  });

  var npaths = paths.length;

  // get (up to) five most common paths
  paths = paths.reduce(function (out, path, index) {
    if (index === 0) out.unshift({ count: 1, path: path });else if (out[0].path[0] === path[0] && out[0].path[1] === path[1]) out[0].count++;else out.unshift({ count: 1, path: path });
    return out;
  }, []);
  ...
  paths = paths.slice(0, 5).map(function (p) {
    return p.path;
  });

  // uniquify the paths
  // always return first path, and don't return paths the same as the previous
  paths = paths.filter(function (p, idx, arr) {
    return idx === 0 || p[0] !== arr[idx - 1][0] || p[1] !== arr[idx - 1][1];
  }).map(function (p) {
    return getPath({ pathDescriptor: p, origin: origin });
  });
  ...
  for (var pidx = 0; pidx < paths.length; pidx++) {
    ...
    if (path.length === 0) {
      ...
      for (var segmentIdx = 0; segmentIdx < path.length; segmentIdx++) {
        var _path$segmentIdx = (0, _slicedToArray3.default)(path[segmentIdx], 3);

        var boardStop = _path$segmentIdx[0];
        var pattern = _path$segmentIdx[1];
        var alightStop = _path$segmentIdx[2];

        // figure out from and to stop indices

        var fromStopId = boardStop + '';
        var toStopId = alightStop + '';

        var index = 0;
        var patternData = network.patterns[pattern + ''];

        while (patternData.stops[index].stop_id !== fromStopId) {
          index++;
        }var fromStopIdx = index;
        while (patternData.stops[index].stop_id !== toStopId) {
          index++;
        }var toStopIdx = index;
  ...
  (0, _assign2.default)(output, network);
...
function getPaths(_ref2) {
  var origin = _ref2.origin;
  var to = _ref2.to;
  var stopTreeCache = _ref2.stopTreeCache;
  var query = _ref2.query;

  var paths = new Array(origin.nMinutes);
  var times = new Uint8Array(origin.nMinutes);
  // fill with non transit time, so we don't include suboptimal transit trips
  var nonTransitTime = (0, _origin.getNonTransitTime)(origin, to);
  (0, _lodash2.default)(times, nonTransitTime);
  ...
  // the way
  if (nonTransitTime < 255) (0, _lodash2.default)(paths, [-1, -1]);

  to.y |= 0;
  to.x |= 0;

  // we're outside the query, bail
  if (to.x >= query.width || to.x < 0 || to.y >= query.height || to.y < 0) return { paths: paths, times: times };

  var stcOffset = stopTreeCache.index[to.y * query.width + to.x];
  var reachableStops = stopTreeCache.data[stcOffset++];

  for (var stopIdx = 0; stopIdx < reachableStops; stopIdx++) {
    var stopId = stopTreeCache.data[stcOffset++];
    var accessTime = stopTreeCache.data[stcOffset++];
    var originOffset = origin.index[stopId];

    // minute, stopTime and pathIdx are delta-coded _per stop_
    for (var minute = 0, stopTime = 0, pathIdx = 0; minute < origin.nMinutes; minute++) {
      stopTime = origin.data[originOffset + minute * _origin.ITERATION_WIDTH];
      pathIdx = origin.data[originOffset + minute * _origin.ITERATION_WIDTH + 3];

      // reachable at this minute
      if (stopTime !== -1) {
        var time = stopTime + accessTime;
...
function getPath(_ref3) {
  var pathDescriptor = _ref3.pathDescriptor;
  var origin = _ref3.origin;

  var _pathDescriptor = (0, _slicedToArray3.default)(pathDescriptor, 2);

  var stop = _pathDescriptor[0];
  var pathIdx = _pathDescriptor[1];

  if (stop === -1) return [];
  // offset to the first path, skipping times and nPaths
  var offset = origin.index[stop] + origin.nMinutes * _origin.ITERATION_WIDTH + 1;

  // seek forward to correct path
  var curIdx = 0;
  while (curIdx++ < pathIdx) {
    var _nSegments = origin.data[offset++];
    offset += _nSegments * 3;
  }

  var nSegments = origin.data[offset++];
  ...
  for (var seg = 0; seg < nSegments; seg++) {
    ...
    path.push((0, _lodash4.default)(origin.data, offset, offset + 3));
    offset += 3;
  }
...
function filterPaths(paths, network) {
  var inPathCount = paths.length;

  // function to convert pattern ID to route ID
  var r = function r(pattern) {
    var pat = network.patterns.find(function (p) {
      return p.pattern_id === String(pattern);
    });
    return network.routes.findIndex(function (r) {
      return r.route_id === pat.route_id;
    });
  };
  ...
  paths.sort(function (p1, p2) {
    if (p1.length < p2.length) return -1;else if (p1.length > p2.length) return 1;else {
  ...
  paths = paths.filter(function (p, i, a) {
    if (i === 0) return true;
    var prev = a[i - 1];
  ...
  var pathCountAfterMultiTransfer = paths.length;
  ...
  paths = paths.filter(function (path, i, a) {
    ...
    try {
      for (var _iterator = (0, _getIterator3.default)(paths), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        try {
          for (var _iterator2 = (0, _getIterator3.default)(otherPath), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        if (otherPathIsSubset) {
          console.log('path ' + otherPath.map(function (seg) {
            return r(seg[1]);
          }) + ' is subset of path ' + routes);
          return false;
        }
      }
    } catch (err) {
...
function bundle(data) {
  ...
  // convert to arrays
  var clusters = data.journeys.map(function (j) {
    return [j];
  });
  var inputSize = data.journeys.length;
  ...
  while (clusters.length > 1) {
    ...
    for (var i = 1; i < clusters.length; i++) {
      for (var j = 0; j < i; j++) {
        var d = clusterDissimilarity(clusters[i], clusters[j], data);
  ...
  data.journeys = clusters.map(function (c) {
    return c.reduce(function (j1, j2) {
      for (var _i = 0; _i < j1.segments.length; _i++) {
        if (j1.segments[_i].type !== 'TRANSIT') continue;

        // convert to pattern groups
        if (!j1.segments[_i].patterns) {
          j1.segments[_i].patterns = [{
            pattern_id: j1.segments[_i].pattern_id,
            from_stop_index: j1.segments[_i].from_stop_index,
            to_stop_index: j1.segments[_i].to_stop_index
          }];
          j1.segments[_i].pattern_id = j1.segments[_i].from_stop_index = j1.segments[_i].to_stop_index = undefined;
        }
        ...
        j1.segments[_i].patterns.push({
          pattern_id: j2.segments[_i].pattern_id,
          from_stop_index: j2.segments[_i].from_stop_index,
          to_stop_index: j2.segments[_i].to_stop_index
        });
      }
...
function clusterDissimilarity(c1, c2, data) {
  ...
  try {
    for (var _iterator3 = (0, _getIterator3.default)(c1), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
      var j1 = _step3.value;
      ...
      try {
        for (var _iterator4 = (0, _getIterator3.default)(c2), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          var j2 = _step4.value;
          ...
          for (var segment = 0; segment < j1.segments.length; segment++) {
            var s1 = j1.segments[segment];
            var s2 = j2.segments[segment];
...
function segmentDissimilarity(s1, s2, data) {
  var pat1 = data.patterns.find(function (p) {
    return p.pattern_id === s1.pattern_id;
  });
  var pat2 = data.patterns.find(function (p) {
    return p.pattern_id === s2.pattern_id;
  });

  var from1 = pat1.stops[s1.from_stop_index].stop_id;
  var to1 = pat1.stops[s1.to_stop_index].stop_id;
  var from2 = pat2.stops[s2.from_stop_index].stop_id;
  var to2 = pat2.stops[s2.to_stop_index].stop_id;

  var d1 = stopDistance(from1, from2, data);
  var d2 = stopDistance(to1, to2, data);
...
function stopDistance(stop1, stop2, data) {
  ...
  var s1 = data.stops.find(function (s) {
    return s.stop_id === stop1 + '';
  });
  var s2 = data.stops.find(function (s) {
    return s.stop_id === stop2 + '';
  });

  var cosLat = Math.cos(s1.stop_lat * Math.PI / 180);
  return Math.pow(s1.stop_lat - s2.stop_lat, 2) + Math.pow(s1.stop_lon * cosLat - s2.stop_lon * cosLat, 2);
}

},{"./mercator":11,"./origin":12,"babel-runtime/core-js/get-iterator":15,"babel-runtime/core-js/object/assign":19,"babel-runtime/helpers/slicedToArray":31,"color":40,"debug":151,"lodash.fill":155,"lodash.slice":156}],8:[function(require,module,exports){
"use strict";
...
function create(data) {
  var array = new Int32Array(data, 4 * 5);
  var header = new Int32Array(data);

  for (var i = 0, prev = 0; i < array.length; i++) {
...
var _webWorkerPromiseInterface = require('web-worker-promise-interface');

var _webWorkerPromiseInterface2 = _interopRequireDefault(_webWorkerPromiseInterface);

var _leaflet = require('leaflet');

var _workerHandlers = require('./worker-handlers');

var _workerHandlers2 = _interopRequireDefault(_workerHandlers);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var imageHeight = 256; // NB raw leaflet map not react-leaflet

var imageWidth = 256;

var Browsochrones = function (_WebWorkerPromiseInte) {
  (0, _inherits3.default)(Browsochrones, _WebWorkerPromiseInte);

  function Browsochrones(opts) {
    (0, _classCallCheck3.default)(this, Browsochrones);

    var _this = (0, _possibleConstructorReturn3.default)(this, (Browsochrones.__proto__ || (0, _getPrototypeOf2.default)(Browsochrones)).call(this, _workerHandlers2.default));

    _this.originLoaded = false;
    _this.query = false;
    _this.stopTreesLoaded = false;
    _this.surfaceLoaded = false;

    var _ref = opts || {};

    var origin = _ref.origin;
    var query = _ref.query;
    var stopTrees = _ref.stopTrees;
    var transitiveNetwork = _ref.transitiveNetwork;


    if (origin) _this.setOrigin(origin.data, origin.point);
    if (query) _this.setQuery(query);
    if (stopTrees) _this.setStopTrees(stopTrees);
    if (transitiveNetwork) _this.setTransitiveNetwork(transitiveNetwork);
  ...
  (0, _createClass3.default)(Browsochrones, [{
    key: 'putGrid',
    value: function putGrid(key, grid) {
      this.work({
        command: 'putGrid',
        message: { key: key, grid: grid }
      });
    }
  }, {
    key: 'drawTile',
    value: function drawTile(canvas, _ref2, zoom) {
      var x = _ref2.x;
      var y = _ref2.y;

      var scaleFactor = Math.pow(2, zoom - this.query.zoom);
      ...
      var xoffset = Math.round(x * imageWidth / scaleFactor - this.query.west);
      var yoffset = Math.round(y * imageHeight / scaleFactor - this.query.north);
      // NB x and y offsets are now relative to query

      var ctx = canvas.getContext('2d');
      var imageData = ctx.createImageData(imageHeight, imageWidth);
    ...
    key: 'getAccessibilityForGrid',
    value: function getAccessibilityForGrid(grid) {
      var cutoff = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 60;

      if (!this.isLoaded()) {
        return _promise2.default.reject(new Error('Accessibility cannot be computed before generating a surface.'));
      }
    ...
    key: 'generateDestinationData',
    value: function generateDestinationData(point) {
      return this.work({
    ...
    key: 'generateTransitiveData',
    value: function generateTransitiveData(point) {
      if (!this.isLoaded()) {
        return _promise2.default.reject(new Error('Transitive data cannot be generated if Browsochrones is not fully loaded.'));
      }
    ...
    value: function generateSurface() {
      ...
      return this.work({
        ...
        _this2.surfaceLoaded = true;
      });
    }
  }, {
    key: 'latLonToOriginPoint',
    value: function latLonToOriginPoint(_ref3) {
      var lat = _ref3.lat;
      var lon = _ref3.lon;
      var lng = _ref3.lng;
      ...
      var north = _query.north;
      var west = _query.west;
      var zoom = _query.zoom;

      var _LeafletMap$prototype = _leaflet.Map.prototype.project([lat, lon], zoom);

      var x = _LeafletMap$prototype.x;
      var y = _LeafletMap$prototype.y;
    ...
    key: 'pixelToOriginPoint',
    value: function pixelToOriginPoint(_ref4, currentZoom) {
      var x = _ref4.x;
      var y = _ref4.y;
      ...
      var north = _query2.north;
      var west = _query2.west;
      var zoom = _query2.zoom;

      var scale = Math.pow(2, zoom - currentZoom);
    ...
    key: 'pointInQueryBounds',
    value: function pointInQueryBounds(_ref5) {
      var x = _ref5.x;
      var y = _ref5.y;
      var _query3 = this.query;
      var height = _query3.height;
      var width = _query3.width;
    ...
    key: 'setOrigin',
    value: function setOrigin(arrayBuffer, point) {
      var _this3 = this;
      ...
      return this.work({
        ...
        _this3.originLoaded = true;
      });
    }
  }, {
    key: 'setQuery',
    value: function setQuery(json) {
      this.query = json;
      return this.work({
        command: 'setQuery',
        message: {
    ...
    key: 'setStopTrees',
    value: function setStopTrees(arrayBuffer) {
      var _this4 = this;

      if (!this.query) {
        return _promise2.default.reject(new Error('Query must be loaded before generating the stop tree cache.'));
      }
      ...
      return this.work({
        ...
        _this4.stopTreesLoaded = true;
      });
    }
  }, {
    key: 'setTransitiveNetwork',
    value: function setTransitiveNetwork(json) {
      return this.work({
...
}(_webWorkerPromiseInterface2.default);

exports.default = Browsochrones;
module.exports = exports['default'];

},{"./worker-handlers":14,"babel-runtime/core-js/object/get-prototype-of":22,"babel-runtime/core-js/promise":24,"babel-runtime/helpers/classCallCheck":27,"babel-runtime/helpers/createClass":28,"babel-runtime/helpers/inherits":29,"babel-runtime/helpers/possibleConstructorReturn":30,"leaflet":154,"web-worker-promise-interface":163}],10:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = isochroneTile;

var _color = require('color');

var _color2 = _interopRequireDefault(_color);

var _lodash = require('lodash.slice');
...
var colorTable = new Uint8Array(255 * 4);

for (var i = 0; i < 254; i++) {
  var val = Math.min(i, 120);

  // 15 minutes of travel time represents 45 degrees on the color wheel
  var color = (0, _color2.default)().hsl(0, 1, 70);
...
function isochroneTile(imageData, _ref) {
  var height = _ref.height;
  var scaleFactor = _ref.scaleFactor;
  var surface = _ref.surface;
  ...
  for (var yp = 0, pixel = 0; yp < imageData.height; yp++) {
    for (var xp = 0; xp < imageData.width; xp++, pixel = pixel + 1 | 0) {
      // figure out where xp and yp fall on the surface
      var xpsurf = xp / scaleFactor + xoffset | 0;
      var ypsurf = yp / scaleFactor + yoffset | 0;

      var _val = void 0;
      if (xpsurf < 0 || xpsurf > width || ypsurf < 0 || ypsurf > height) {
        _val = 255;
      } else {
        _val = surface[ypsurf * width + xpsurf];
      }

      var _colorScheme = _colorScheme || function (val) {
        return (0, _lodash2.default)(colorTable, val * 4, val * 4 + 4);
      };

      var col = _colorScheme(_val);
...
},{"color":40,"lodash.slice":156}],11:[function(require,module,exports){
"use strict";
...
exports.lonToPixel = lonToPixel;
exports.latToPixel = latToPixel;
exports.pixelToLon = pixelToLon;
exports.pixelToLat = pixelToLat;
...
function latToPixel(lat, zoom) {
  var invCos = 1 / Math.cos(radians(lat));
  var tan = Math.tan(radians(lat));
  var ln = Math.log(tan + invCos);
  // return int
...
function pixelToLat(y, zoom) {
  var tile = y / 256;
  return degrees(Math.atan((0, _sinh2.default)(Math.PI - tile * Math.PI * 2 / Math.pow(2, zoom))));
}
...
},{"babel-runtime/core-js/math/sinh":18}],12:[function(require,module,exports){
"use strict";
...
exports.create = create;
exports.getNonTransitTime = getNonTransitTime;
/**
...
var ITERATION_WIDTH = exports.ITERATION_WIDTH = 4;

function create(data, point) {
  var origin = {};
  ...
  var nPixels = Math.pow(origin.radius * 2 + 1, 2);

  for (var pixel = 0, nonTransitTime = 0; pixel < nPixels; pixel++) {
  ...
  // build index
  var nStops = data[offset++];
  var nMinutes = data[offset++];
  origin.nStops = nStops;
  origin.nMinutes = nMinutes;

  origin.index = new Int32Array(nStops);

  for (var stop = 0; stop < nStops; stop++) {
    origin.index[stop] = offset;

    // de delta code times and paths
    for (var min = 0, tvlTime = 0, inVehicleTravelTime = 0, waitTime = 0, path = 0; min < nMinutes; min++) {
    ...
    // read paths
    var nPaths = data[offset++];

    for (var _path = 0; _path < nPaths; _path++) {
...
function getNonTransitTime(origin, _ref) {
  var x = _ref.x;
  var y = _ref.y;
  ...
  if (Math.abs(relx) <= origin.radius && Math.abs(rely) <= origin.radius) {
    ...
    var timeSecs = origin.data[rely * (origin.radius * 2 + 1) + relx + 1];
    var timeMins = timeSecs / 60 | 0;
...
function create(data, size) {
  ...
  for (var i = 0, pixel = 0, stopId = 0, time = 0; i < data.length; pixel++) {
...
var _jsolines = require('jsolines');

var _jsolines2 = _interopRequireDefault(_jsolines);

var _webWorkerPromiseInterface = require('web-worker-promise-interface');

var _accessibilityForGrid2 = require('./accessibility-for-grid');

var _accessibilityForGrid3 = _interopRequireDefault(_accessibilityForGrid2);

var _getSurface = require('./get-surface');

var _getSurface2 = _interopRequireDefault(_getSurface);

var _getTransitiveData = require('./get-transitive-data');

var _getTransitiveData2 = _interopRequireDefault(_getTransitiveData);

var _isochroneTile = require('./isochrone-tile');

var _isochroneTile2 = _interopRequireDefault(_isochroneTile);

var _grid = require('./grid');

var _mercator = require('./mercator');

var mercator = _interopRequireWildcard(_mercator);

var _origin = require('./origin');

var _stopTreeCache = require('./stop-tree-cache');

function _interopRequireWildcard(obj) { if (obj && obj.__esModule) { return obj; } else { var newObj = {}; if (obj != null) { for (var key in obj) { if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key]; } } newObj.default = obj; return newObj; } }

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

module.exports = (0, _webWorkerPromiseInterface.createHandler)({
  putGrid: function putGrid(ctx, message) {
    if (!ctx.grids) ctx.grids = new _map2.default();
    ctx.grids.set(message.key, (0, _grid.create)(message.grid));
  },
  accessibilityForGrid: function accessibilityForGrid(ctx, message) {
    return (0, _accessibilityForGrid3.default)({
      grid: ctx.grids.get(message.grid),
      cutoff: message.cutoff,
      surface: ctx.surface
    });
  },
  drawTile: function drawTile(ctx, message) {
    return (0, _isochroneTile2.default)(message.imageData, {
  ...
  setOrigin: function setOrigin(ctx, message) {
    ctx.origin = (0, _origin.create)(new Int32Array(message.arrayBuffer), message.point);
  },
  setQuery: function setQuery(ctx, message) {
    ctx.query = message.query;
  },
  setStopTreeCache: function setStopTreeCache(ctx, message) {
    ctx.stopTreeCache = (0, _stopTreeCache.create)(new Int32Array(message.arrayBuffer), ctx.query.width * ctx.query.height);
  },
  setTransitiveNetwork: function setTransitiveNetwork(ctx, message) {
    ctx.transitiveNetwork = message.network;
  },
  generateSurface: function generateSurface(ctx, message) {
    ctx.surface = (0, _getSurface2.default)({
      cutoff: message.cutoff,
      origin: ctx.origin,
      query: ctx.query,
  ...
  generateDestinationData: function generateDestinationData(ctx, message) {
    ...
    return {
      ...
      transitive: (0, _getTransitiveData2.default)({
  ...
  generateTransitiveData: function generateTransitiveData(ctx, message) {
    ctx.transitiveData = (0, _getTransitiveData2.default)({
      network: ctx.transitiveNetwork,
      origin: ctx.origin,
      query: ctx.query,
      stopTreeCache: ctx.stopTreeCache,
  ...
  getContour: function getContour(ctx, message) {
    ctx.contour = (0, _jsolines.getContour)({
      cutoff: message.cutoff,
      height: ctx.query.height,
      surface: ctx.surface.surface,
      width: ctx.query.width
  ...
  getIsochrone: function getIsochrone(ctx, message) {
    return (0, _jsolines2.default)({
      surface: ctx.surface.surface,
      width: ctx.query.width,
      height: ctx.query.height,
      cutoff: message.cutoff,
  ...
  getPath: function getPath(ctx, message) {
    return (0, _getTransitiveData.getPath)({
      pathDescriptor: message.path,
      origin: ctx.origin
    });
  },
  getPaths: function getPaths(ctx, message) {
    return (0, _getTransitiveData.getPaths)({
      origin: ctx.origin,
      query: ctx.query,
      stopTreeCache: ctx.stopTreeCache,
      to: message.point
...
},{"./accessibility-for-grid":5,"./get-surface":6,"./get-transitive-data":7,"./grid":8,"./isochrone-tile":10,"./mercator":11,"./origin":12,"./stop-tree-cache":13,"babel-runtime/core-js/map":17,"babel-runtime/helpers/slicedToArray":31,"jsolines":153,"web-worker-promise-interface":163}],15:[function(require,module,exports){
module.exports = { "default": require("core-js/library/fn/get-iterator"), __esModule: true };
},{"core-js/library/fn/get-iterator":41}],16:[function(require,module,exports){
module.exports = { "default": require("core-js/library/fn/is-iterable"), __esModule: true };
},{"core-js/library/fn/is-iterable":42}],17:[function(require,module,exports){
module.exports = { "default": require("core-js/library/fn/map"), __esModule: true };
},{"core-js/library/fn/map":43}],18:[function(require,module,exports){
module.exports = { "default": require("core-js/library/fn/math/sinh"), __esModule: true };
},{"core-js/library/fn/math/sinh":44}],19:[function(require,module,exports){
module.exports = { "default": require("core-js/library/fn/object/assign"), __esModule: true };
},{"core-js/library/fn/object/assign":45}],20:[function(require,module,exports){
module.exports = { "default": require("core-js/library/fn/object/create"), __esModule: true };
},{"core-js/library/fn/object/create":46}],21:[function(require,module,exports){
module.exports = { "default": require("core-js/library/fn/object/define-property"), __esModule: true };
},{"core-js/library/fn/object/define-property":47}],22:[function(require,module,exports){
...
},{"../core-js/object/create":20,"../core-js/object/set-prototype-of":23,"../helpers/typeof":32}],30:[function(require,module,exports){
"use strict";
...
},{"../helpers/typeof":32}],31:[function(require,module,exports){
"use strict";
...
},{"../core-js/get-iterator":15,"../core-js/is-iterable":16}],32:[function(require,module,exports){
"use strict";
...
},{"./css-keywords":35}],35:[function(require,module,exports){
module.exports = {
...
},{"./conversions":34,"./route":37}],37:[function(require,module,exports){
var conversions = require('./conversions');
...
},{"./conversions":34}],38:[function(require,module,exports){
module.exports = {
	"aliceblue": [240, 248, 255],
	"antiquewhite": [250, 235, 215],
	"aqua": [0, 255, 255],
...
},{"color-name":38}],40:[function(require,module,exports){
/* MIT license */
...
},{"clone":33,"color-convert":36,"color-string":39}],41:[function(require,module,exports){
require('../modules/web.dom.iterable');
require('../modules/es6.string.iterator');
module.exports = require('../modules/core.get-iterator');
},{"../modules/core.get-iterator":133,"../modules/es6.string.iterator":145,"../modules/web.dom.iterable":150}],42:[function(require,module,exports){
require('../modules/web.dom.iterable');
require('../modules/es6.string.iterator');
module.exports = require('../modules/core.is-iterable');
},{"../modules/core.is-iterable":134,"../modules/es6.string.iterator":145,"../modules/web.dom.iterable":150}],43:[function(require,module,exports){
...
},{"../modules/_core":67,"../modules/es6.map":136,"../modules/es6.object.to-string":143,"../modules/es6.string.iterator":145,"../modules/es7.map.to-json":147,"../modules/web.dom.iterable":150}],44:[function(require,module,exports){
require('../../modules/es6.math.sinh');
module.exports = require('../../modules/_core').Math.sinh;
},{"../../modules/_core":67,"../../modules/es6.math.sinh":137}],45:[function(require,module,exports){
require('../../modules/es6.object.assign');
module.exports = require('../../modules/_core').Object.assign;
},{"../../modules/_core":67,"../../modules/es6.object.assign":138}],46:[function(require,module,exports){
require('../../modules/es6.object.create');
var $Object = require('../../modules/_core').Object;
module.exports = function create(P, D){
  return $Object.create(P, D);
};
},{"../../modules/_core":67,"../../modules/es6.object.create":139}],47:[function(require,module,exports){
require('../../modules/es6.object.define-property');
var $Object = require('../../modules/_core').Object;
module.exports = function defineProperty(it, key, desc){
  return $Object.defineProperty(it, key, desc);
};
},{"../../modules/_core":67,"../../modules/es6.object.define-property":140}],48:[function(require,module,exports){
require('../../modules/es6.object.get-prototype-of');
module.exports = require('../../modules/_core').Object.getPrototypeOf;
},{"../../modules/_core":67,"../../modules/es6.object.get-prototype-of":141}],49:[function(require,module,exports){
require('../../modules/es6.object.set-prototype-of');
module.exports = require('../../modules/_core').Object.setPrototypeOf;
},{"../../modules/_core":67,"../../modules/es6.object.set-prototype-of":142}],50:[function(require,module,exports){
require('../modules/es6.object.to-string');
require('../modules/es6.string.iterator');
require('../modules/web.dom.iterable');
require('../modules/es6.promise');
module.exports = require('../modules/_core').Promise;
},{"../modules/_core":67,"../modules/es6.object.to-string":143,"../modules/es6.promise":144,"../modules/es6.string.iterator":145,"../modules/web.dom.iterable":150}],51:[function(require,module,exports){
require('../../modules/es6.symbol');
require('../../modules/es6.object.to-string');
require('../../modules/es7.symbol.async-iterator');
require('../../modules/es7.symbol.observable');
module.exports = require('../../modules/_core').Symbol;
},{"../../modules/_core":67,"../../modules/es6.object.to-string":143,"../../modules/es6.symbol":146,"../../modules/es7.symbol.async-iterator":148,"../../modules/es7.symbol.observable":149}],52:[function(require,module,exports){
require('../../modules/es6.string.iterator');
require('../../modules/web.dom.iterable');
module.exports = require('../../modules/_wks-ext').f('iterator');
},{"../../modules/_wks-ext":130,"../../modules/es6.string.iterator":145,"../../modules/web.dom.iterable":150}],53:[function(require,module,exports){
...
},{"./_to-index":122,"./_to-iobject":124,"./_to-length":125}],59:[function(require,module,exports){
// 0 -> Array#forEach
// 1 -> Array#map
// 2 -> Array#filter
// 3 -> Array#some
...
},{"./_array-species-create":61,"./_ctx":68,"./_iobject":83,"./_to-length":125,"./_to-object":126}],60:[function(require,module,exports){
var isObject = require('./_is-object')
...
},{"./_is-array":85,"./_is-object":86,"./_wks":131}],61:[function(require,module,exports){
...
},{"./_cof":63,"./_wks":131}],63:[function(require,module,exports){
...
},{"./_an-instance":55,"./_ctx":68,"./_defined":69,"./_descriptors":70,"./_for-of":76,"./_iter-define":89,"./_iter-step":91,"./_meta":96,"./_object-create":99,"./_object-dp":100,"./_redefine-all":112,"./_set-species":115}],65:[function(require,module,exports){
...
},{"./_an-instance":55,"./_array-methods":59,"./_descriptors":70,"./_export":74,"./_fails":75,"./_for-of":76,"./_global":77,"./_hide":79,"./_is-object":86,"./_meta":96,"./_object-dp":100,"./_redefine-all":112,"./_set-to-string-tag":116}],67:[function(require,module,exports){
...
},{"./_object-gops":105,"./_object-keys":108,"./_object-pie":109}],74:[function(require,module,exports){
var global    = require('./_global')
  , core      = require('./_core')
  , ctx       = require('./_ctx')
  , hide      = require('./_hide')
...
},{"./_core":67,"./_ctx":68,"./_global":77,"./_hide":79}],75:[function(require,module,exports){
...
},{"./_an-object":56,"./_ctx":68,"./_is-array-iter":84,"./_iter-call":87,"./_to-length":125,"./core.get-iterator-method":132}],77:[function(require,module,exports){
...
},{"./_descriptors":70,"./_object-dp":100,"./_property-desc":111}],80:[function(require,module,exports){
...
},{"./_iterators":92,"./_wks":131}],85:[function(require,module,exports){
...
},{"./_hide":79,"./_object-create":99,"./_property-desc":111,"./_set-to-string-tag":116,"./_wks":131}],89:[function(require,module,exports){
'use strict';
...
},{"./_export":74,"./_has":78,"./_hide":79,"./_iter-create":88,"./_iterators":92,"./_library":94,"./_object-gpo":106,"./_redefine":113,"./_set-to-string-tag":116,"./_wks":131}],90:[function(require,module,exports){
...
},{"./_wks":131}],91:[function(require,module,exports){
...
},{"./_object-keys":108,"./_to-iobject":124}],94:[function(require,module,exports){
module.exports = true;
...
},{"./_fails":75,"./_has":78,"./_is-object":86,"./_object-dp":100,"./_uid":128}],97:[function(require,module,exports){
var global    = require('./_global')
...
},{"./_cof":63,"./_global":77,"./_task":121}],98:[function(require,module,exports){
'use strict';
...
},{"./_fails":75,"./_iobject":83,"./_object-gops":105,"./_object-keys":108,"./_object-pie":109,"./_to-object":126}],99:[function(require,module,exports){
...
},{"./_an-object":56,"./_dom-create":71,"./_enum-bug-keys":72,"./_html":80,"./_object-dps":101,"./_shared-key":117}],100:[function(require,module,exports){
var anObject       = require('./_an-object')
...
},{"./_an-object":56,"./_descriptors":70,"./_ie8-dom-define":81,"./_to-primitive":127}],101:[function(require,module,exports){
var dP       = require('./_object-dp')
  , anObject = require('./_an-object')
...
},{"./_an-object":56,"./_descriptors":70,"./_object-dp":100,"./_object-keys":108}],102:[function(require,module,exports){
...
},{"./_descriptors":70,"./_has":78,"./_ie8-dom-define":81,"./_object-pie":109,"./_property-desc":111,"./_to-iobject":124,"./_to-primitive":127}],103:[function(require,module,exports){
...
},{"./_object-gopn":104,"./_to-iobject":124}],104:[function(require,module,exports){
...
},{"./_enum-bug-keys":72,"./_object-keys-internal":107}],105:[function(require,module,exports){
...
},{"./_has":78,"./_shared-key":117,"./_to-object":126}],107:[function(require,module,exports){
var has          = require('./_has')
  , toIObject    = require('./_to-iobject')
...
},{"./_array-includes":58,"./_has":78,"./_shared-key":117,"./_to-iobject":124}],108:[function(require,module,exports){
...
},{"./_enum-bug-keys":72,"./_object-keys-internal":107}],109:[function(require,module,exports){
...
},{"./_core":67,"./_export":74,"./_fails":75}],111:[function(require,module,exports){
...
},{"./_hide":79}],113:[function(require,module,exports){
...
},{"./_an-object":56,"./_ctx":68,"./_is-object":86,"./_object-gopd":102}],115:[function(require,module,exports){
'use strict';
var global      = require('./_global')
  , core        = require('./_core')
...
},{"./_core":67,"./_descriptors":70,"./_global":77,"./_object-dp":100,"./_wks":131}],116:[function(require,module,exports){
...
},{"./_has":78,"./_object-dp":100,"./_wks":131}],117:[function(require,module,exports){
...
},{"./_global":77}],119:[function(require,module,exports){
...
},{"./_a-function":53,"./_an-object":56,"./_wks":131}],120:[function(require,module,exports){
...
},{"./_cof":63,"./_ctx":68,"./_dom-create":71,"./_global":77,"./_html":80,"./_invoke":82}],122:[function(require,module,exports){
var toInteger = require('./_to-integer')
  , max       = Math.max
  , min       = Math.min;
...
},{"./_core":67,"./_global":77,"./_library":94,"./_object-dp":100,"./_wks-ext":130}],130:[function(require,module,exports){
...
$exports.store = store;
},{"./_global":77,"./_shared":118,"./_uid":128}],132:[function(require,module,exports){
var classof   = require('./_classof')
  , ITERATOR  = require('./_wks')('iterator')
  , Iterators = require('./_iterators');
...
},{"./_classof":62,"./_core":67,"./_iterators":92,"./_wks":131}],133:[function(require,module,exports){
var anObject = require('./_an-object')
...
},{"./_an-object":56,"./_core":67,"./core.get-iterator-method":132}],134:[function(require,module,exports){
var classof   = require('./_classof')
  , ITERATOR  = require('./_wks')('iterator')
  , Iterators = require('./_iterators');
...
},{"./_classof":62,"./_core":67,"./_iterators":92,"./_wks":131}],135:[function(require,module,exports){
'use strict';
...
},{"./_add-to-unscopables":54,"./_iter-define":89,"./_iter-step":91,"./_iterators":92,"./_to-iobject":124}],136:[function(require,module,exports){
'use strict';
...
},{"./_collection":66,"./_collection-strong":64}],137:[function(require,module,exports){
...
},{"./_export":74,"./_fails":75,"./_math-expm1":95}],138:[function(require,module,exports){
// 19.1.3.1 Object.assign(target, source)
var $export = require('./_export');

$export($export.S + $export.F, 'Object', {assign: require('./_object-assign')});
},{"./_export":74,"./_object-assign":98}],139:[function(require,module,exports){
var $export = require('./_export')
// 19.1.2.2 / 15.2.3.5 Object.create(O [, Properties])
$export($export.S, 'Object', {create: require('./_object-create')});
},{"./_export":74,"./_object-create":99}],140:[function(require,module,exports){
var $export = require('./_export');
// 19.1.2.4 / 15.2.3.6 Object.defineProperty(O, P, Attributes)
$export($export.S + $export.F * !require('./_descriptors'), 'Object', {defineProperty: require('./_object-dp').f});
},{"./_descriptors":70,"./_export":74,"./_object-dp":100}],141:[function(require,module,exports){
...
},{"./_object-gpo":106,"./_object-sap":110,"./_to-object":126}],142:[function(require,module,exports){
// 19.1.3.19 Object.setPrototypeOf(O, proto)
var $export = require('./_export');
$export($export.S, 'Object', {setPrototypeOf: require('./_set-proto').set});
},{"./_export":74,"./_set-proto":114}],143:[function(require,module,exports){
...
},{"./_a-function":53,"./_an-instance":55,"./_classof":62,"./_core":67,"./_ctx":68,"./_export":74,"./_for-of":76,"./_global":77,"./_is-object":86,"./_iter-detect":90,"./_library":94,"./_microtask":97,"./_redefine-all":112,"./_set-species":115,"./_set-to-string-tag":116,"./_species-constructor":119,"./_task":121,"./_wks":131}],145:[function(require,module,exports){
'use strict';
...
},{"./_iter-define":89,"./_string-at":120}],146:[function(require,module,exports){
'use strict';
...
},{"./_an-object":56,"./_descriptors":70,"./_enum-keys":73,"./_export":74,"./_fails":75,"./_global":77,"./_has":78,"./_hide":79,"./_is-array":85,"./_keyof":93,"./_library":94,"./_meta":96,"./_object-create":99,"./_object-dp":100,"./_object-gopd":102,"./_object-gopn":104,"./_object-gopn-ext":103,"./_object-gops":105,"./_object-keys":108,"./_object-pie":109,"./_property-desc":111,"./_redefine":113,"./_set-to-string-tag":116,"./_shared":118,"./_to-iobject":124,"./_to-primitive":127,"./_uid":128,"./_wks":131,"./_wks-define":129,"./_wks-ext":130}],147:[function(require,module,exports){
// https://github.com/DavidBruant/Map-Set.prototype.toJSON
var $export  = require('./_export');

$export($export.P + $export.R, 'Map', {toJSON: require('./_collection-to-json')('Map')});
},{"./_collection-to-json":65,"./_export":74}],148:[function(require,module,exports){
require('./_wks-define')('asyncIterator');
},{"./_wks-define":129}],149:[function(require,module,exports){
...
},{"./_global":77,"./_hide":79,"./_iterators":92,"./_wks":131,"./es6.array.iterator":135}],151:[function(require,module,exports){
...
},{"./debug":152}],152:[function(require,module,exports){
...
},{"ms":157}],153:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = jsolines;
exports.getContour = getContour;
...
var debug = (0, _debug2.default)('jsolines');
...
function jsolines(_ref) {
  var surface = _ref.surface;
  var width = _ref.width;
  var height = _ref.height;
  var cutoff = _ref.cutoff;
  var project = _ref.project;
  var _ref$interpolation = _ref.interpolation;
  var interpolation = _ref$interpolation === undefined ? true : _ref$interpolation;

  // first, create the contour grid
  var contour = getContour({ surface: surface, width: width, height: height, cutoff: cutoff });
  var cWidth = width - 1;

  // javascript does not have boolean arrays. lame.
  var found = new Uint8Array((width - 1) * (height - 1));
  ...
  for (var origy = 0; origy < height - 1; origy++) {
    for (var origx = 0; origx < width - 1; origx++) {
      if (found[origy * cWidth + origx] === 1) {
        continue;
      }

      var idx = contour[origy * cWidth + origx];
      ...
      while (true) {
        ...
        var topLeft = surface[y * width + x];
        var topRight = surface[y * width + x + 1];
        var botLeft = surface[(y + 1) * width + x];
        var botRight = surface[(y + 1) * width + x + 1];

        // do linear interpolation
        var coord = void 0;
        if (startx < x) {
          // came from left
          var frac = interpolation ? (cutoff - topLeft) / (botLeft - topLeft) : 0.5;
          ...
          // came from right
          var _frac = interpolation ? (cutoff - topRight) / (botRight - topRight) : 0.5;
          ...
          // came from bottom
          var _frac2 = interpolation ? (cutoff - botLeft) / (botRight - botLeft) : 0.5;
          ...
          // came from top
          var _frac3 = interpolation ? (cutoff - topLeft) / (topRight - topLeft) : 0.5;
        ...
        if (x === origx && y === origy) {
          ...
          if (direction > 0) shells.push(geom);else holes.push(geom);
          break;
        }
      }
    }
  ...
  return {
    ...
    geometry: {
      type: 'MultiPolygon',
      coordinates: shells.map(function (s) {
...
function getContour(_ref2) {
  var surface = _ref2.surface;
  var width = _ref2.width;
  var height = _ref2.height;
  var cutoff = _ref2.cutoff;

  var contour = new Uint8Array((width - 1) * (height - 1));

  // compute contour values for each cell
  for (var x = 0; x < width - 1; x++) {
    for (var y = 0; y < height - 1; y++) {
      var topLeft = surface[y * width + x] < cutoff;
      var topRight = surface[y * width + x + 1] < cutoff;
      var botLeft = surface[(y + 1) * width + x] < cutoff;
      var botRight = surface[(y + 1) * width + x + 1] < cutoff;
...
},{"debug":151,"turf-inside":158,"turf-point":160}],154:[function(require,module,exports){
/*
...
},{"turf-invariant":159}],159:[function(require,module,exports){
/**
...
var WebWorkerPromiseInterface = function () {
  function WebWorkerPromiseInterface(handler) {
    var _this = this;

    (0, _classCallCheck3.default)(this, WebWorkerPromiseInterface);

    this.__errorHandlers = [];
    this.__messageHandlers = [];
    this.__worker = (0, _webworkify2.default)(handler);
    this.__worker.addEventListener('message', function (event) {
      _this.__messageHandlers.forEach(function (handler) {
        return handler.fn(event.data);
      });
    });
    this.__worker.addEventListener('error', function (error) {
      _this.__errorHandlers.forEach(function (handler) {
        return handler.fn(error);
      });
    });
  }

  (0, _createClass3.default)(WebWorkerPromiseInterface, [{
    key: 'work',
    value: function work(_ref) {
      ...
      var transferrable = _ref.transferrable;

      return new _promise2.default(function (resolve, reject) {
        var id = _uuid2.default.v4();
        var clear = function clear() {
          _this2.__messageHandlers = _this2.__messageHandlers.filter(function (h) {
            return h.id !== id;
          });
          _this2.__errorHandlers = _this2.__errorHandlers.filter(function (h) {
            return h.id !== id;
          });
        };

        _this2.__messageHandlers.push({
        ...
        _this2.__worker.postMessage({ command: command, id: id, message: message }, transferrable);
      });
    }
  }]);
  return WebWorkerPromiseInterface;
}();

exports.default = WebWorkerPromiseInterface;
function createHandler(functions) {
  return function handler(self) {
    ...
    self.addEventListener('message', function (event) {
      ...
      var command = _event$data.command;
      var id = _event$data.id;
      var message = _event$data.message;

      _promise2.default.resolve(functions[command].call(null, cache, message)).then(function (results) {
        self.postMessage({
          command: command,
          id: id,
          message: results
...
},{"babel-runtime/core-js/promise":24,"babel-runtime/helpers/classCallCheck":27,"babel-runtime/helpers/createClass":28,"uuid":162,"webworkify":164}],164:[function(require,module,exports){

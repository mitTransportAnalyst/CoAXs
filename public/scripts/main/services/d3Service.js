...
this.drawGraph = function (cutoff, plotData, indicator) {
	...
	xScale2 = d3.scale.ordinal()
        .domain(indicators2)
        .rangeBands([width/3,width/3+(indicators2.length*width/3)], .2),
  ...
  vis1.append("svg:path")
	...
	vis1.append('g')
		...
		.attr("class","stat")
		.attr("y", 25-margins.top)
        .attr("x", xScale2(indicators2[0])+(indicators2.length)*xScale2.rangeBand()+34)
		.style("text-anchor","start")
		.style("opacity", 0.75)
        .html("Total (2011 data)");
